...
- Route control exposed through the local API is named `API route control`. Do not name this workflow after the current client application, such as Jupyter or notebook. `pause` means a pause request; `pause_ack` means the external API loop has actually reached a safe waiting point; only then may GUI controls show Resume. While a pause request is pending, the same control must remain an Interrupt path.
...
- Do not hardcode tunable measurement criteria deep inside implementation code. Thresholds that affect contact classification, contact seek, short/open decisions, compliance handling, stability checks, or route pass/fail behavior must be represented as named configuration, exposed through the relevant API/client/notebook path, and written into result metadata when practical.
