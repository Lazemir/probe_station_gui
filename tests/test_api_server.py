...
class ApiServerHttpTest(unittest.TestCase):
    def _client(self, *, move_callback=None, status_callback=None):
        ...
        server = ProbeStationApiServer(
            move_callback=move_callback or (lambda targets: {"accepted": True, "targets": targets}),
    ...
    def test_move_endpoint_passes_json_body_targets_to_callback(self) -> None:
        ...
        client = self._client(
            move_callback=lambda targets: (
                calls.append(targets) or {"accepted": True, "targets": targets}
            )
        )

        response = client.post(
            "/api/v1/stage/move",
            json={"coordinates": {"x": "1.25"}, "z": 3.0},
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(calls, [{"X": 1.25, "Z": 3.0}])
        self.assertEqual(response.json()["targets"], {"X": 1.25, "Z": 3.0})

    def test_move_endpoint_rejects_empty_or_rejected_moves(self) -> None:
        client = self._client(
            move_callback=lambda _targets: {
                "accepted": False,
                "status_code": 409,
                "message": "busy",
            }
