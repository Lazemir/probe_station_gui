...
def test_alignment_capture_running_disables_duplicates_but_keeps_cancel(
    ...
    panel.set_design_marks(((0.0, 0.0), (10.0, 0.0)))
    panel.set_captured_points(((1.0, 2.0), None))
    panel.set_pick_slot(0)

    panel.set_capture_running(True)

    assert panel.capture_button(0).isEnabled() is False
    assert panel.capture_button(1).isEnabled() is False
    ...
    assert panel.capture_button(0).isEnabled() is True
    assert panel.capture_button(1).isEnabled() is True
    assert panel._capture_mode_combo.isEnabled() is True
    assert panel._reset_points_button.isEnabled() is True
    assert panel._cancel_pick_button.isEnabled() is True
    panel.close()
