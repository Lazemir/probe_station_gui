...
@pytest.mark.parametrize(
    "axis_value", [np.array(["X"]), np.array(1), np.array("x"), np.array("Q")]
)
...
def test_import_rejects_invalid_curve(
    tmp_path, controller, physical, message: str
) -> None:
...
def test_import_rejects_malformed_numeric_arrays(tmp_path) -> None:
    ...
    np.savez(
        path, axis=np.array("X"), controller=["left", "right"], physical=[0.0, 1.0]
    )
