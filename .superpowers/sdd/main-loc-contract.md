...
- Task 43 passed the contract: `main.py LOC 7961 -> 7720`, SLOC `7413 -> 7172`, Wily `main.py` cyclomatic `1502 -> 1451`, MI `0 -> 0`. The pass moved serial/controller connection flow behind a main-window module while preserving wrapper method names and serial/LCR connection state semantics.
- Task 44 passed the contract and completes the Phase 1 `main.py` reduction track: `main.py LOC 7720 -> 7545`, SLOC `7172 -> 6997`, Wily `main.py` cyclomatic `1451 -> 1385`, MI `0 -> 0`. The pass moved homing UI/limit-axis handling and shutdown ordering behind main-window adapter modules, restored the `_queue_or_start_homing_axes(...)` override seam after review, and added direct tests for synchronous homing-start emission and full shutdown order.
