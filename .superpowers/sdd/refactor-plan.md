...
Current completed task: Task 49c, GW Instek session adapter extraction.
Active next task: Task 49d, LCR controller runtime/worker extraction.
...
1. Extract one remaining controller-runtime responsibility from `lcr.py` while preserving `LCRMeterController` as the public facade.
2. Preferred next seam: worker queue / idle waiting / worker wakeup mechanics, because this is still mixed with instrument capability and route measurement behavior.
3. Preserve all Qt signal behavior, polling stop/resume behavior, and Keithley live output context semantics.
4. Add or update characterization tests around any worker/runtime path touched before moving it.
5. Target `lcr.py < 1400 LOC` for 49d, without replacing logic with shallow pass-through wrappers.
