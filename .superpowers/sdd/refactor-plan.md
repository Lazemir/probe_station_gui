...
Current completed task: Task 48e, route measurement profile apply cleanup.
Active next task: Task 49, LCR module lifecycle and capability split.
...
1. Investigate `probe_station_gui/instruments/meters/lcr.py` before editing; identify worker lifecycle, live polling, meter capability, and route-meter adapter responsibilities.
2. Use subagents for independent exploration/review if there are separable seams, then verify their findings locally before editing.
3. Start with characterization tests around the first seam touched; preserve public meter and route adapter behavior.
4. Target file below 1400 lines without moving complexity into shallow pass-through modules.
5. Run Wily metrics from a disposable UTF-8 temp clone/cache, focused instrument tests, full `pytest tests`, configured ruff, coverage, and lizard before each commit.
