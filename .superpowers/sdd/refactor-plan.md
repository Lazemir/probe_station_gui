...
Current completed task: Task 48a, route measurement presentation widget extraction.
Active next task: Task 48b, route measurement profile persistence extraction.
...
| 48 | `dialogs/route_measurement_dialog.py` | Qt construction, profile persistence, run controls, histogram/raw data display live together. | Split profile persistence and presentation modules; keep dialog as Qt adapter. | Route dialog/run UI tests and route measurement tests. | In progress: 48a extracted SI-prefix and histogram/raw-data presentation (`route_measurement_dialog.py 2179 -> 1809 LOC`, Wily cyclomatic `324 -> 249`, new widget/result modules with direct characterization tests). Remaining target: extract profile persistence and/or operation enablement so the dialog drops below 1600 lines; remaining warnings are `_update_operation_state`, `_apply_profile_data`, and localized histogram `paintEvent`. |
...
1. Extract route measurement profile persistence/apply helpers from `RouteMeasurementDialog` into a focused module or mixin.
2. Do not touch Pause Request/Pause Ack/Interrupt semantics unless adding explicit route-control regression coverage.
3. Add focused characterization tests before changing runtime controls or profile application behavior.
...
5. Task 48 target remains: `route_measurement_dialog.py < 1600 LOC`; route Pause/Resume/Interrupt semantics remain safety-critical and must not change incidentally.
