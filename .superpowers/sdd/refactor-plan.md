...
Current completed task: Task 48d, route measurement histogram paint cleanup.
Active next task: Task 48e, route measurement profile apply cleanup.
...
1. Split profile `_apply_profile_data` only if it can be improved behind existing profile tests without behavior churn.
2. Prefer a smaller data-mapping helper over more shallow methods; this pass should improve max-function complexity and avoid worsening aggregate Wily metrics if practical.
