...
Current completed task: Task 48c, route measurement operation-state policy extraction.
Active next task: Task 48d, route measurement histogram paint cleanup.
...
1. Split histogram `paintEvent` drawing helpers inside `route_measurement_result_views.py` if it reduces warning-level complexity without adding indirection.
2. Keep profile `_apply_profile_data` as a follow-up only if it can be split behind existing profile tests without behavior churn.
3. Do not touch Pause Request/Pause Ack/Interrupt semantics unless adding explicit route-control regression coverage.
...
5. Task 48 size target is met; route Pause/Resume/Interrupt semantics remain safety-critical and must not change incidentally.
