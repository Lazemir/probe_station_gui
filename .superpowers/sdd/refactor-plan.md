...
Current completed task: Task 52a, stage A-axis/needles test split.
Active next task: Task 52b, continue splitting stage controller tests.
...
| `tests/stage/test_controller.py` | 2751 | stage controller tests are partially split; startup/absolute/status/autofocus/objective/jog/motion/status-refresh/startup-sync/reconnect groups remain together |
...
Continue Phase 3 with Task 52b:

1. Use a read-only explorer first to choose the next contiguous responsibility split from the remaining `tests/stage/test_controller.py`.
2. Candidate groups: startup limits/feed override, absolute/status/autofocus/objective, jog/motion safety, or status refresh/startup sync/reconnect.
3. Reuse `tests/stage/controller_test_support.py`; avoid importing split tests from each other.
4. Preserve direct file execution and module-style `unittest` execution for split files.
5. Run focused stage split files, direct execution where applicable, full `pytest tests`, configured ruff, Wily/radon/lizard metrics, and commit before the next split.
