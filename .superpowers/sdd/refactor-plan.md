...
Current completed task: Task 48b, route measurement profile persistence extraction.
Active next task: Task 48c, route measurement operation-state and histogram cleanup.
...
1. Split or simplify route measurement operation enablement without changing route-control semantics.
2. Split histogram `paintEvent` drawing helpers inside `route_measurement_result_views.py` if it reduces warning-level complexity without adding indirection.
3. Keep profile `_apply_profile_data` as a follow-up only if it can be split behind existing profile tests without behavior churn.
