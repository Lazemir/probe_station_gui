...
- Lizard after: `Main._start_route_measurement` 52 NLOC, CCN 7, length 52.
- Radon before: `Main._start_route_measurement` D (23).
- Radon after: `Main._start_route_measurement` B (7).
- Wily detail: `main.py:Main._start_route_measurement` cyclomatic complexity 23 -> 7.
...
- Lizard after:
  ...
  - `Main._build_route_measurement_runner`: 59 NLOC, CCN 1, length 73.
...
- After:
  - `main.py`: NLOC 11656, AvgCCN 4.6, function count 524.
  - `probe_station_gui\route\session_start.py`: NLOC 500, AvgCCN 2.8, function count 17.
  - Total for `main.py probe_station_gui\route`: NLOC 19912, AvgCCN 4.0, function count 952, warning count 83.

### Wily diff against `1ec14ef`

- `main.py`: Cyclomatic Complexity 2400 -> 2404; Lines of Code 12218 -> 12287; Maintainability Index 0 -> 0.
...
- Function detail:
  - `main.py:Main._start_route_measurement`: 23 -> 7.
  - New `Main` adapters: 5, 5, 4, 3, 2, 1.
...
- Residual metric note: aggregate route-module LOC and complexity increased because pure decision helpers and tests were added; the targeted GUI method complexity dropped materially.
