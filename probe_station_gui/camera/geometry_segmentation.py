"""Metal geometry segmentation and topology feature extraction."""
...
def segment_metal_geometry(frame: object) -> GeometryMaskFrame:
    ...
    if len(features) < _MIN_GEOMETRY_FEATURES:
...
def _component_geometry_and_features(
    ...
    *,
    ...
    geometry = GeometryMaskComponent(
        component_id=component_id,
        bounding_box_px=bounding_box,
        area_px=area,
        position_px=position,
    ...
    return geometry, tuple(features)
...
def _skeleton_neighbour_offsets(
    skeleton: np.ndarray,
    x: int,
    y: int,
) -> tuple[tuple[int, int], ...]:
...
def _line_center_samples(
    ...
    return tuple(
        ...
        for x, y in zip(
            xs[::_LINE_CENTER_SAMPLE_SPACING_PX],
            ys[::_LINE_CENTER_SAMPLE_SPACING_PX],
        )
    )
