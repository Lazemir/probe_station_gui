...
class AlignmentPanel(QWidget):
    ...
    def _refresh_ui(self, *_unused: object) -> None:
        ...
        self._mode_label.setText(
            "Mode: design-linked" if using_design_marks else "Mode: quick"
        )
        ...
        if self._capture_running and self._pick_slot is not None:
            self._status_label.setText(f"Capturing S{self._pick_slot + 1}.")
        ...
        for slot, (design_label, stage_label, button) in enumerate(
            ...
            button.setEnabled(
                ...
                and (not using_design_marks or slot < len(self._design_marks))
            )
