...
def on_stage_axis_editing_finished(
    ...
    if not stage_position_panel.gui_coordinate_motion_editing_enabled(owner):
        ...
        owner._pending_coordinate_motion_lease = None
    ...
    def reject(message: str, timeout_ms: int) -> bool:
        ...
        if not owner._pending_stage_axis_targets:
            owner._pending_coordinate_motion_lease = None
    ...
    if motion_lease is not None:
        ...
        candidate_targets = {
            pending_axis: pending[1]
            for pending_axis, pending in owner._pending_stage_axis_targets.items()
        }
        ...
        projection = owner._project_gui_coordinate_motion(
            ...
            mode="G90",
            lease=(
                getattr(owner, "_pending_coordinate_motion_lease", None)
                or motion_lease
            ),
        )
    ...
    if motion_lease is not None:
        owner._pending_coordinate_motion_lease = projected_lease
...
def apply_pending_stage_coordinate_targets(owner: Any) -> None:
    ...
    for axis in owner.STAGE_AXIS_NAMES:
        ...
        if field is not None and field.isEnabled() and field.isModified():
            if owner._on_stage_axis_editing_finished(axis) is False:
                had_error = True
    if had_error:
        owner._update_stage_coordinate_apply_state()
        return
    ...
    if motion_lease is not None:
        display_targets = tuple(
            (axis, values[1])
            for axis, values in owner._pending_stage_axis_targets.items()
        )
        projection = owner._project_gui_coordinate_motion(
            display_targets,
            mode="G90",
        ...
        targets = dict(owner._pending_stage_axis_targets)
    owner.view.setFocus(Qt.OtherFocusReason)
